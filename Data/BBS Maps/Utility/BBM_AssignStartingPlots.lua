...
function BBM_AssignStartingPlots.Create(args)
    ...
    if BBS_Success then
        ...
        if useQuadrantDistribution then
            ...
            _Debug("Using standard random placement for city states")
    ...
    for _, tile in ipairs(validTiles) do
        if not tile.IsCivStartingPlot then
            ...
            elseif tile.quadrant == Quadrant_NE then
                table.insert(regionTiles[2], tile)
            elseif tile.quadrant == Quadrant_SE then
                table.insert(regionTiles[3], tile)
            elseif tile.quadrant == Quadrant_SW then
                table.insert(regionTiles[4], tile)
            end
        end
    end
